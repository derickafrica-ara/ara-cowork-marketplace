...
| **Recipient allow-list (drafts)** | `APPLE_MAIL_DRAFT_ALLOWED_DOMAINS` (MCP env) | `ara-data.com,falkecorp.com,falkehoa.com` | **Named business domains only — ARA + the two Falke client domains** (widened 2026-07-18, Derick's decision, so client-bound drafts can be staged in Mail). Bounds who an injection could ever draft to: an explicit named-domain list, never a wildcard; the code's fail-closed default stays `ara-data.com` only, so a lost/empty env **narrows** back to ARA-only. Personal contacts (Gmail/iCloud/iMessage) are never valid nudge recipients. Keep conservative — add a domain only by deliberate decision. |
